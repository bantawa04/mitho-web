...
import { mockAdminNotifications, mockAdminOperator } from "@/features/admin/data/admin-data"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-brand-deep-green/10 bg-white text-brand-dark-green shadow-sm transition-colors hover:border-brand-deep-green/18 hover:bg-muted"
              >
            ...
            <DropdownMenuContent align="end" className="w-[22rem] rounded-xl border-brand-deep-green/10 bg-white p-2 shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
                ...
                return (
                  <DropdownMenuItem key={item.id} asChild className="rounded-xl px-3 py-3">
                    <Link href={item.href} className="items-start gap-3">
                      <span className="mt-0.5 flex h-8 w-8 items-center justify-center rounded-full bg-muted text-brand-dark-green">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-2 block text-xs font-medium text-muted-foreground">{item.when}</span>
                      </span>
                    </Link>
                  </DropdownMenuItem>
                )

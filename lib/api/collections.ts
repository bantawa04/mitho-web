...
function mapCreatePayload(payload: CreateCollectionPayload) {
  return {
    ...
    initialItem: payload.initialItem
      ? {
          businessId: payload.initialItem.businessId,
...
export async function listCollections(
  ...
  const { data } = await API.get<ISuccessResponse<CollectionsListResponse>>(
    "/collections",
    {
...
export async function getCollection(id: string): Promise<CollectionRecord> {
  const { data } = await API.get<ISuccessResponse<CollectionRecord>>(
    `/collections/${id.trim()}`,
  )
  return data.data
}
...
export async function createCollection(
  ...
  const { data } = await API.post<ISuccessResponse<CollectionRecord>>(
    "/collections",
...
export async function updateCollection(
  ...
  const { data } = await API.put<ISuccessResponse<CollectionRecord>>(
    `/collections/${id.trim()}`,
...
export async function updateCollectionItem(
  ...
  const { data } = await API.put<ISuccessResponse<CollectionRecord>>(
    `/collections/${collectionId.trim()}/items/${itemId.trim()}`,
    {
      note: payload.note || undefined,
    },
  )
...
export async function deleteCollectionItem(
  ...
  const { data } = await API.delete<ISuccessResponse<CollectionRecord>>(
    `/collections/${collectionId.trim()}/items/${itemId.trim()}`,
  )
  return data.data
}
...
export async function reorderCollectionItems(
  ...
  const { data } = await API.put<ISuccessResponse<CollectionRecord>>(
    `/collections/${collectionId.trim()}/items/reorder`,
    {
...
export async function copyCollection(
  ...
  const { data } = await API.post<ISuccessResponse<CollectionRecord>>(
    `/collections/${collectionId.trim()}/copy`,
  )
  return data.data
}
...
export async function listPublicCollections(
  username: string,
  params?: Pick<ListCollectionsParams, "page" | "perPage">,
): Promise<CollectionsListResponse> {
  const { data } = await API.get<ISuccessResponse<CollectionsListResponse>>(
...
export async function getPublicCollection(
  ...
  const { data } = await API.get<ISuccessResponse<CollectionRecord>>(
    `/users/${username}/collections/${id.trim()}`,
  )
  return data.data
}
